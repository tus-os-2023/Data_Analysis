...
def translate_column_names(data):
    ...
    translated_columns = [
        ...
        "MaximumSnowDepth",
        "MaximumSnowDepthNoPhenomenonInformation",
        "AverageWindSpeed", "AverageVaporPressure",
        "AverageHumidity",
        "AverageSeaLevelPressure",
        "AverageGroundLevelPressure", "AverageCloudCover",
        "AverageTemperature", "TotalSolarRadiation",
        "TotalPrecipitation",
        "TotalPrecipitationNoPhenomenonInformation",
        "TotalSnowfall",
        "TotalSnowfallNoPhenomenonInformation",
        "MaximumTemperature", "MinimumTemperature",
        "MostFrequentWindDirection", "MaximumWindSpeed",
        "MaximumWindSpeedWindDirection",
        "LowestSeaLevelPressure",
        "LowestSeaLevelPressureNoPhenomenonInformation",
        "MinimumRelativeHumidity",
        "MaximumPrecipitationin10Minutes",
        "MaximumPrecipitationin10MinutesNoPhenomenonInformation",
        "MaximumInstantaneousWindSpeed",
        "MaximumInstantaneousWindSpeedWindDirection",
        "WeatherSummaryDay", "WeatherSummaryNight"
    ]

    data.columns = [
        translated_columns[
            original_columns.index(col)
        ] if col in original_columns else col for col in data.columns
    ]
...
def aggregate_csv_files(folder_path, start_year, num_years):
    ...
    compiled_filename = f'Compiled_{start_year}-{start_year+num_years}_eng.csv'
    result.to_csv(
        os.path.join(folder_path, compiled_filename),
        index=False, encoding='utf-8'
    )

    print(f"Saved as {compiled_filename}")
